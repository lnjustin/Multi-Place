            ...
            header()
                ...
                if (state.accessories) {
                    for (id in state.accessories) {
                        paragraph '<table width="100%"><tr><td align="center"><font style="font-size:20px;font-weight: bold">' + formatAccessoryPreview(id) + settings["accessory${id}Name"] + '</font></td></tr></table>', width:3
                    }
                    paragraph getInterface("line", "")
                }
                ...
                paragraph getInterface("subHeader", " Add Accessory")                
                    ...
                    input name: "accessory${state.lastAccessoryID}NumStarsRequired", type: "number", title: "Number of Stars to Earn Accessory", submitOnChange: false, required: true
                     ...
                     if (state.people) {
                         state.people.each { id, person ->
                              def avatar = getPersonAvatar(id)
                              paragraph '<table width="100%"><tr><td align="center"><font style="font-size:20px;font-weight: bold">' + formatAccessoryPreview(avatar) + settings["person${id}Name"] + '</font></td></tr></table>', width:12
                    ...
                    input name: "accessory${accessoryId}NumStarsRequired", type: "number", title: "Number of Stars to Earn Accessory", submitOnChange: false, required: true
                     ...
                     if (state.people) {
                        state.people.each { id, person ->
                              def avatar = getPersonAvatar(id)
                              def avatarPreview = avatar ? avatar : getPathOfStandardIcon("Dark", "Unknown")
                              paragraph '<table width="100%"><tr><td align="center"><font style="font-size:20px;font-weight: bold">' + formatAccessoryPreview(avatarPreview) + settings["person${id}Name"] + '</font></td></tr></table>', width:12
...
def getAccessory(accessoryId) {
    if (accessoryId == null) return null
     ...
     if (state.people) {
          state.people.each { id, person ->
              def personMap = [top: settings["accessory${accessoryId}TopPerson${id}"], bottom: settings["accessory${accessoryId}BottomPerson${id}"], left: settings["accessory${accessoryId}LeftPerson${id}"], right: settings["accessory${accessoryId}RightPerson${id}"], scale: settings["accessory${accessoryId}ScalePerson${id}"], isEarned: settings["isAccessory${accessoryId}EarnedByPerson${id}"], isNext: settings["isNextAccessory${accessoryId}ForPerson${id}"]]
...
def getNumStarsNeededForNextAccessory(personId, excludeEarned = true) {
    ...
    if (nextAccessory == null) log.warn "No next accessory for person ${personId} set up yet."
    return nextAccessory.numStarsRequired ? nextAccessory.numStarsRequired : 0
}
...
def handleTap(personId) {
    if (state.people && state.people[personId]) {
        ...
        else if (state.people[personId].stars.earned == getNumStarsNeededForNextAccessory(personId)) {
...
def cycleActiveAccessory(personId, category = null) {       
    ...
    if (activeAccessories.size() == 0 && earnedAccessories.size() > 0) state.people[personId].accessories?.active.add(earnedAccessories[0]) // no active accessory (in the category), so set the first earned accessory (in the category) to active
    else {
        for (def i = 0; i < earnedAccessories.size(); i++) {
            if (activeAccessories.contains(earnedAccessories[i])) {
                 state.people[personId].accessories?.active.removeElement(earnedAccessories[i])
                 if (i < earnedAccessories.size() - 1) state.people[personId].accessories?.active.add(earnedAccessories[i+1])
...
def takeBackNextAccessory(personId) {
     def nextAccessoryId = getNextAccessoryIdForPerson(personId, false)  
     if (nextAccessoryId != null) {
         state.people[personId].accessories?.earned.removeElement(nextAccessoryId)
         if (state.people[personId].accessories?.active.contains(nextAccessoryId)) {
             state.people[personId].accessories?.active.removeElement(nextAccessoryId)
             def nextAccessory = getAccessory(nextAccessoryId) 
             def accessoriesInCategory = getEarnedAccessoryIDs(personId, nextAccessory.category)
...
def subtractStar(personId) {
    if (state.people[personId].stars.earned == 0) {
        ...
        takeBackNextAccessory(personId)
